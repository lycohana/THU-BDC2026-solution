...
def add_cross_section_features(df, date_col='日期'):
    ...
    valid_cols = [c for c in CORE_CS_COLS if c in out.columns]
    for col in valid_cols:
        out[f'cs_rank_{col}'] = out.groupby(date_col)[col].rank(method='average', pct=True)

        med = out.groupby(date_col)[col].transform('median')
        mad = out.groupby(date_col)[col].transform(_mad)
        out[f'cs_rz_{col}'] = ((out[col] - med) / (1.4826 * mad + 1e-9)).clip(-5.0, 5.0)

    if 'return_1' in out.columns:
        out['mkt_ret_1'] = out.groupby(date_col)['return_1'].transform('mean')
        out['mkt_breadth_1'] = out.groupby(date_col)['return_1'].transform(lambda x: (x > 0).mean())
        out['mkt_dispersion_1'] = out.groupby(date_col)['return_1'].transform('std').fillna(0.0)
        out['alpha_rel_1'] = out['return_1'] - out['mkt_ret_1']
    if 'return_5' in out.columns:
        out['mkt_ret_5'] = out.groupby(date_col)['return_5'].transform('mean')
        out['alpha_rel_5'] = out['return_5'] - out['mkt_ret_5']
    if '成交额' in out.columns:
        out['mkt_amt_mean'] = out.groupby(date_col)['成交额'].transform('mean')
        out['mkt_amt_std'] = out.groupby(date_col)['成交额'].transform('std').fillna(0.0)
...
def create_ranking_dataset_vectorized(
    ...
    # 2. 确保每只股票都有 'label'。标签构建时已经要求未来 target_horizon
    # 个交易日存在，因此这里不再额外检查窗口结束后的行数。
    ...
    # - 窗口结束行已有 label
    # 注意：标签本身是按 T+1 到 T+5 交易日开盘收益构建的，
    # 因此不能再用自然日连续性过滤，否则会错误丢弃跨周末/节假日样本。
    ...
    if min_window_end_date is not None:
        ...
        window_df = window_df[window_df['date'] >= min_window_end_date]
